...
function makePart(overrides?: Partial<DocReferencesPart>): DocReferencesPart {
  return {
    id: 'part1',
    itemId: 'item1',
...
describe('DocReferencesPartFeatureComponent', () => {
  ...
  async function configure(
    loadResult: EditedObject<DocReferencesPart> | null | undefined = undefined,
  ...
  beforeEach(async () => {
    await configure({
      value: makePart({ references: [{ citation: 'Cic. Att. 1.1' }] }),
      thesauri: {},
    });
  });
  ...
  it('should set identity from the route', () => {
    ...
    expect(component.identity().typeId).toBe(DOC_REFERENCES_PART_TYPEID);
  });

  it('should load data on init and request the doc-references thesauri', async () => {
    ...
    expect(component.data()?.value).toEqual(
      makePart({ references: [{ citation: 'Cic. Att. 1.1' }] }),
    );
  });
  ...
  it('save should delegate to the editor service', async () => {
    const part = makePart({ references: [{ citation: 'Verg. Aen. 1.1' }] });
    component.save(part);
