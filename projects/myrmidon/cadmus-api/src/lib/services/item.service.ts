...
export class ItemService {
  ...
  /**
   * Get a user-facing message from an HTTP error response, using the
   * details of the problem returned by the server when available.
  ...
  /**
   * Download the item with the specified ID as a JSON file, which can
   * later be uploaded via uploadItem.
   * @param id The item's ID.
   * @param parts True to include the item's parts (default).
   * @returns Observable with the JSON file content. On error, the
   * observable fails with a user-facing message.
   */
  public downloadItem(id: string, parts = true): Observable<Blob> {
    return this._http
      .get(`${this._env.get('apiUrl')}items/${id}/download`, {
        params: new HttpParams().set('parts', parts.toString()),
        responseType: 'blob',
      })
      .pipe(
        catchError((error: HttpErrorResponse) =>
          throwError(() => ItemService.getErrorMessage(error)),
        ),
      );
  }

  /**
   * Upload the specified JSON file, as generated by downloadItem, to add
   * a new item with its parts. The upload fails if an item with the same
   * ID already exists; if the item or any of its parts were deleted, they
   * get a new ID.
   * @param file The JSON file.
   * @returns Observable with the imported item, including its parts with
   * their final IDs. On error, the observable fails with a user-facing
   * message.
   */
  public uploadItem(file: File): Observable<Item> {
    const data = new FormData();
    data.append('file', file, file.name);

    return this._http
      .post<Item>(`${this._env.get('apiUrl')}items/upload`, data)
      .pipe(
        catchError((error: HttpErrorResponse) =>
          throwError(() => ItemService.getErrorMessage(error)),
        ),
