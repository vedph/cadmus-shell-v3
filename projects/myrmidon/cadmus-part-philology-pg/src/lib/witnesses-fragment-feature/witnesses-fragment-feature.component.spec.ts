...
function makeFragment(
  overrides?: Partial<WitnessesFragment>,
): WitnessesFragment {
  return {
    location: '1.1',
    witnesses: [{ id: 'w1', citation: '1.1', text: 'alpha' }],
    ...overrides,
  };
}
...
function makeLayerPart(overrides?: Partial<TextLayerPart>): TextLayerPart {
  return {
    ...
    roleId: WITNESSES_FRAGMENT_TYPEID,
...
describe('WitnessesFragmentFeatureComponent', () => {
  ...
  async function configure(
    loadResult: EditedObject<WitnessesFragment> | null | undefined = undefined,
  ) {
    ...
    await TestBed.configureTestingModule({
      imports: [WitnessesFragmentFeatureComponent],
      providers: [
        { provide: Router, useValue: router },
        {
          provide: ActivatedRoute,
          useValue: mockFragmentRoute({ frTypeId: WITNESSES_FRAGMENT_TYPEID }),
        },
  ...
  it('should set identity from the route', () => {
    ...
    expect(component.identity().frTypeId).toBe(WITNESSES_FRAGMENT_TYPEID);
  });
  ...
  describe('save', () => {
    it('should replace the fragment at the current location and delegate to the editor service', async () => {
      ...
      const updated = makeFragment({
        witnesses: [{ id: 'w2', citation: '1.1', text: 'beta' }],
      });
  ...
  describe('close', () => {
    it('should resolve the editor key from the layer part and navigate accordingly', async () => {
      ...
      expect(router.navigate).toHaveBeenCalledWith(
        ...
        { queryParams: { rid: WITNESSES_FRAGMENT_TYPEID } },
      );
    });
  });
});
